...
import ToggleArrowIconDown from '../../assets/icons/ToggleArrowIconDown';
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View>
        <View>
          <Image
            source={require('./components/images/imagecard.png')}
            style={styles.image_background}
          />
          <View style={styles.header_txt_img}>
            <Text style={styles.header_text_1}>
              Welcome to the Curiosity Oasis!
            </Text>
            <Text style={styles.header_text_2}>
              Your go-to destination for unraveling the mysteries of recycling!
            </Text>
          </View>
        </View>
      ...
      <View style={styles.box_question}>
        ...
        <TouchableOpacity onPress={() => toggleRectangle(0)} style={styles.box}>
          {showRectangle[0] ? <ToggleArrowIconDown /> : <ToggleArrowIcon />}
        </TouchableOpacity>
      </View>
      {showRectangle[0] && (
        <View style={styles.long_box}>
          <Text style={styles.long_text}>
            ...
            ipsum ad laudantium cumque sapiente aliquam, voluptate labore
            consequatur accusamus id repudiandae quas necessitatibus! Sed
            asperiores expedita tenetur beatae incidunt illo?
          </Text>
        </View>
      )}

      <View style={styles.box_question_1}>
        <Text style={styles.text_box_question}>
          How do I know if an item is recyclable or not?
        </Text>
        <TouchableOpacity onPress={() => toggleRectangle(1)} style={styles.box}>
          {showRectangle[1] ? <ToggleArrowIconDown /> : <ToggleArrowIcon />}
        </TouchableOpacity>
      </View>
      {showRectangle[1] && (
        <View style={styles.long_box}>
      ...
      <View style={styles.box_question_2}>
        <Text style={styles.text_box_question}>
          What are eco hotspots and where do I find them?
        </Text>
        <TouchableOpacity onPress={() => toggleRectangle(2)} style={styles.box}>
          {showRectangle[2] ? <ToggleArrowIconDown /> : <ToggleArrowIcon />}
        </TouchableOpacity>
      </View>
      {showRectangle[2] && (
        <View style={styles.long_box}>
      ...
      <View style={styles.box_question_3}>
        <Text style={styles.text_box_question}>
          How do I earn more eco-coins?
        </Text>
        <TouchableOpacity onPress={() => toggleRectangle(3)} style={styles.box}>
          {showRectangle[3] ? <ToggleArrowIconDown /> : <ToggleArrowIcon />}
        </TouchableOpacity>
      </View>
      {showRectangle[3] && (
        <View style={styles.long_box}>
      ...
      <View style={styles.box_question_4}>
        <Text style={styles.text_box_question}>
          How can I redeem my eco-coins for rewards?
        </Text>
        <TouchableOpacity onPress={() => toggleRectangle(4)} style={styles.box}>
          {showRectangle[4] ? <ToggleArrowIconDown /> : <ToggleArrowIcon />}
        </TouchableOpacity>
      </View>
      {showRectangle[4] && (
        <View style={styles.long_box}>
      ...
      <View style={styles.box_question_5}>
        <Text style={styles.text_box_question}>
          Can I cancel or change a reward redemption after it's been processed?
        </Text>
        <TouchableOpacity onPress={() => toggleRectangle(5)} style={styles.box}>
          {showRectangle[5] ? <ToggleArrowIconDown /> : <ToggleArrowIcon />}
        </TouchableOpacity>
      </View>
      {showRectangle[5] && (
        <View style={styles.long_box}>
